...
export default function DeviceSelector({ isDarkMode = false }) {
    ...
    const themeStyles = {
        ...
        buttonBg: isDarkMode ? 'bg-gray-700 hover:bg-gray-600 border-gray-600' : 'bg-gray-100 hover:bg-gray-200 border-gray-300',
        buttonText: isDarkMode ? 'text-gray-200' : 'text-gray-700',
        hrColor: isDarkMode ? 'border-gray-600' : 'border-gray-200',
        checkboxAccent: isDarkMode ? 'text-blue-400 focus:ring-blue-400' : 'text-blue-600 focus:ring-blue-500'
    }
    ...
    return (
        <div className={`${themeStyles.cardBg} rounded-lg border ${themeStyles.cardBorder} shadow-sm p-4 transition-colors duration-300`}>
            ...
            <div className="mt-3">
                {devices.length === 0 && (
                    <div className={`${themeStyles.emptyText} text-sm transition-colors duration-300`}>
                ...
                {devices.map(id => (
                    ...
                    >
                        <label className="flex items-center gap-2 cursor-pointer">
                            ...
                            />
                            <span className={`${themeStyles.primaryText} transition-colors duration-300`}>
                                {id}
                            </span>
                        </label>
                        <small className={`${themeStyles.mutedText} text-xs transition-colors duration-300`}>
                            live
                        </small>
                    </div>
                ))}
            </div>
            <hr className={`my-4 ${themeStyles.hrColor} transition-colors duration-300`} />

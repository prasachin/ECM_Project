...
export default function DataTable({ isDarkMode = false }) {
  ...
  return (
    <div
      className={`${themeStyles.cardBg} rounded-lg border ${themeStyles.cardBorder} shadow-sm p-4 mt-3 transition-colors duration-300`}
    >
      <h3
      ...
      <div className="overflow-x-auto rounded-lg mt-2">
        <table className="w-full border-collapse min-w-[700px]">
          <thead>
            <tr
              className={`border-b ${themeStyles.headerBorder} transition-colors duration-300`}
            >
              <th
                className={`text-left py-2 px-1 ${themeStyles.headerText} font-medium transition-colors duration-300`}
              >
                Device
              </th>
              <th
                className={`text-center py-2 px-1 ${themeStyles.headerText} font-medium transition-colors duration-300`}
              >
                Power (W)
              </th>
              <th
          ...
          <tbody>
            {selected.length === 0 && (
              <tr>
                <td
                  colSpan={7}
                  className={`py-4 px-1 ${themeStyles.emptyText} text-sm transition-colors duration-300`}
                >
            ...
            {selected.map((id) => {
              ...
              return (
                <tr
                  key={id}
                  className={`border-b ${themeStyles.rowBorder} last:border-b-0 ${themeStyles.rowHover} transition-colors duration-200`}
                >
                  <td className={`py-2 px-1 ${themeStyles.primaryText}`}>
                    {id}
                  </td>
                  <td
                    className={`text-center py-2 px-1 ${themeStyles.primaryText}`}
                  >
                    {last ? last.power_w : "—"}
                  </td>
                  <td
